...
def round_scores(student_scores):
    ...
    for score in student_scores:
        ...
        rounded_student_scores.append(score)
...
def count_failed_students(student_scores):
    ...
    failures = [std for std in student_scores if std <= 40]
...
def letter_grades(highest):
    ...
    iteration = 1
    while iteration < 4:
        initial_grade = initial_grade + difference
        grades.append(initial_grade)
        iteration += 1
...
def student_ranking(student_scores, student_names):
    ...
    for index, score in enumerate(student_scores):
        student_rank = f'{index + 1}. {student_names[index]}: {score}'
...
def perfect_score(student_info):
    ...
    for i in range(len(student_info)):
        if student_info[i][1] == 100:
            perfect_student.append(student_info[i][0])
            perfect_student.append(student_info[i][1])
            break
